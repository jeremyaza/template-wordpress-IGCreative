 ...
 */
function test_register_option( $wp_customize, $option ) {
 ...
 */
function test_sort_sections_list( $wp_customize ) {
	foreach ( $wp_customize->sections() as $section_key => $section_object ) {
		if ( false !== strpos( $section_key, 'test_' ) && 'test_important_links' !== $section_key ) {
			$options[] = $section_key;
		}
	}
...
add_action( 'customize_register', 'test_sort_sections_list', 99 );
 ...
 * @since 1.0
 */
function test_section_visibility_options() {
	$options = array(
		'homepage'    => esc_html__( 'Homepage / Frontpage', 'test' ),
		'entire-site' => esc_html__( 'Entire Site', 'test' ),
		'disabled'    => esc_html__( 'Disabled', 'test' ),
	);

	return apply_filters( 'test_section_visibility_options', $options );
}
 ...
 * @since 1.0
 */
function test_get_pagination_types() {
	$pagination_types = array(
		'default' => esc_html__( 'Default(Older Posts/Newer Posts)', 'test' ),
		'numeric' => esc_html__( 'Numeric', 'test' ),
	);

	return apply_filters( 'test_get_pagination_types', $pagination_types );
}
 ...
 * @return post_array
 */
function test_generate_post_array( $post_type = 'post' ) {
	$output = array();
	...
	$output['0']= esc_html__( '-- Select --', 'test' );

	foreach ( $posts as $post ) {
		$output[ $post->ID ] = ! empty( $post->post_title ) ? $post->post_title : sprintf( __( '#%d (no title)', 'test' ), $post->ID );
	}
