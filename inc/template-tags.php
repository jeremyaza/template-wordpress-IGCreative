...
if ( ! function_exists( 'test_posted_on' ) ) :
	...
	function test_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Publicado el %s', 'post date', 'test' ),
...
if ( ! function_exists( 'test_posted_by' ) ) :
	...
	function test_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'por %s', 'post author', 'test' ),
	...
	function test_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( '%1$s', 'test' ) . '</span>', '<div class="content-tags">' . $tags_list . '</div>');
			}
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ' ', 'test' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Categorias: %1$s', 'test' ) . '</span>', '<div class="content-categories">' . $categories_list . '</div>'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

		}

		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link"> <span class="text-edit-comment">Comentarios: </span>';
			comments_popup_link(
				sprintf(
					wp_kses(
